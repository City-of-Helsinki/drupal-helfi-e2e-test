...
const fillRegularDaysOffPerMonth = (page: Page, value: string) => {
  test.step('Fill regular days off per month with off range value', async () => {
    ...
    const inputs = page.locator(
      '[data-slot-number] .hds-selection-group__items [name*="free_days"]',
    );
...
test('On submit, unfilled fields give an error', async ({ page }) => {
  ...
  for (let i = 1; i <= 4; i++) {
    const slot = page.locator(
      `[data-slot-number="${i}"] .hds-selection-group__items`,
    );
  ...
  await expectVisible(
    page.getByText(
      'Lukumäärän pitäisi olla vähintään 5, jos samassa osoitteessa asuu aikuinen ja 4 lasta.',
      { exact: true },
    ),
  );
  await expectVisible(page.getByRole('link', { name: 'Perheen koko' }));
...
test('Household size must be two or more', async ({ page }) => {
  ...
  await clickResultsButton(page);
  await expectVisible(
    page.getByText('Arvon pitää olla 2 tai enemmän: Perheen koko.'),
  );
...
test('Regular days off per month must be positive and in range, or empty', async ({
  page,
}) => {
  ...
  await expectVisible(
    page.getByText(
      'Arvon pitää olla väliltä 0 ja 12: Säännöllisiä vapaapäiviä kuukaudessa (päivää)',
    ),
  );
