...
test('Vacation money input must be positive and in range, or empty', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Laske arvio' }).click();
  expect(await page.getByText('Arvon pitää olla väliltä 0 ja 90: Lomaraha.').isVisible()).toBeTruthy();
  ...
  await page.getByRole('button', { name: 'Laske arvio' }).click();
  expect(await page.getByText('Arvon pitää olla väliltä 0 ja 90: Lomaraha.').isVisible()).toBeFalsy();
