...
test('Monthly pay input must be positive and in range', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Laske arvio' }).click();
  expect(
    await page.getByText(`Arvon pitää olla väliltä 0 ja 10000: ${translations.label_monthly_pay.fi}.`).isVisible(),
  ).toBeTruthy();
...
test('Vacation money input must be positive and in range, or empty', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Laske arvio' }).click();
  expect(
    await page.getByText(`Arvon pitää olla väliltä 0 ja 90: ${translations.label_vacation_money.fi}.`).isVisible(),
  ).toBeTruthy();
