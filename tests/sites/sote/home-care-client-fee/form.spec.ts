...
test('Household size input must be positive, integer and in range', async ({ page }) => {
  ...
  expect(await page.getByText('Arvon pitää olla 1 tai enemmän: Talouden koko.').isVisible()).toBeTruthy();
  ...
  expect(await page.getByText('Arvon pitää olla 1 tai enemmän: Talouden koko.').isVisible()).toBeFalsy();
});
...
test('Monthly usage input must be positive, integer and in range', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Laske arvio' }).click();
  expect(
    await page.getByText('Arvon pitää olla väliltä 0 ja 744: Kotihoidon tuntimäärä kuukaudessa.').isVisible(),
  ).toBeTruthy();
  ...
  await page.getByRole('button', { name: 'Laske arvio' }).click();
  expect(
    await page.getByText('Kenttään on syötettävä kokonaisluku: Kotihoidon tuntimäärä kuukaudessa.').isVisible(),
  ).toBeTruthy();
});
...
test('If meal service is selected, a new input must become visible and it should be required, integer and in range', async ({
  ...
  expect(await page.getByText('Kenttä on pakollinen: Aterioiden määrä viikossa.').isVisible()).toBeTruthy();
  ...
  expect(await page.getByText('Arvon pitää olla väliltä 1 ja 7: Aterioiden määrä viikossa.').isVisible()).toBeTruthy();
  ...
  expect(
    await page.getByText('Kenttään on syötettävä kokonaisluku: Aterioiden määrä viikossa.').isVisible(),
  ).toBeTruthy();
  ...
  expect(await page.getByText('Arvon pitää olla väliltä 1 ja 7: Aterioiden määrä viikossa.').isVisible()).toBeFalsy();
});
