...
test('Service provider price per hour input must be positive and in range', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Laske arvio' }).click();
  expect(
    await page.getByText('Arvon pitää olla 0 tai enemmän: Palveluntuottajan tuntihinta.').isVisible(),
  ).toBeTruthy();

  await page.getByLabel('Palveluntuottajan tuntihinta (euroa)').fill('100€');
  await page.getByRole('button', { name: 'Laske arvio' }).click();

  expect(await page.getByText('Arvon pitää olla 0 tai enemmän: Palveluntuottajan tuntihinta.').isVisible()).toBeFalsy();
});
