...
test('Net income per month input must be positive and in range', async ({ page }) => {
  ...
  expect(
    await page.getByText('Arvon pitää olla 0 tai enemmän: Hakijan nettotulot kuukaudessa.').isVisible(),
  ).toBeTruthy();
...
test('Service provider price per 24h input must be positive and in range', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Laske arvio' }).click();
  expect(
    await page.getByText('Arvon pitää olla 0 tai enemmän: Palveluasumisen vuorokausihinta.').isVisible(),
  ).toBeTruthy();
