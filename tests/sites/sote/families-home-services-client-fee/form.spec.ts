...
test('Household size input must be positive, integer and in range', async ({ page }) => {
  ...
  expect(await page.getByText('Arvon pitää olla väliltä 1 ja 6: Perheen koko.').isVisible()).toBeTruthy();
  ...
  await page.getByRole('button', { name: 'Laske arvio' }).click();
  expect(await page.getByText('Kenttään on syötettävä kokonaisluku: Perheen koko.').isVisible()).toBeTruthy();

  await page.getByLabel('Perheen koko (henkilöä)').fill('6');
  await page.getByRole('button', { name: 'Laske arvio' }).click();

  expect(await page.getByText('Arvon pitää olla väliltä 1 ja 6: Perheen koko.').isVisible()).toBeFalsy();
});
...
test('Gross income per month input must be positive and in range', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Laske arvio' }).click();
  expect(
    await page.getByText('Arvon pitää olla 0 tai enemmän: Perheen bruttotulot kuukaudessa.').isVisible(),
  ).toBeTruthy();

  await page.getByLabel('Perheen bruttotulot kuukaudessa (euroa)').fill('1000,10€');
  await page.getByRole('button', { name: 'Laske arvio' }).click();
...
test('Monthly usage input must be positive, integer and in range', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Laske arvio' }).click();
  expect(
    await page.getByText('Arvon pitää olla väliltä 0 ja 744: Palvelutunteja kuukaudessa.').isVisible(),
  ).toBeTruthy();
  ...
  await page.getByRole('button', { name: 'Laske arvio' }).click();
  expect(
    await page.getByText('Kenttään on syötettävä kokonaisluku: Palvelutunteja kuukaudessa.').isVisible(),
  ).toBeTruthy();
